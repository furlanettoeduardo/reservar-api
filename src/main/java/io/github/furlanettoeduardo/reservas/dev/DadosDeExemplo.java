...
public class DadosDeExemplo {
    ...
    @Bean
    ApplicationRunner semearDadosDeExemplo(EspacoRepositorio espacos,
                                           ClienteRepositorio clientes,
                                           ReservaRepositorio reservas,
                                           TransactionTemplate transacao) {
        return argumentos -> transacao.executeWithoutResult(
                status -> semear(espacos, clientes, reservas));
    }

    private void semear(EspacoRepositorio espacos, ClienteRepositorio clientes,
                        ReservaRepositorio reservas) {
        if (espacos.contar() > 0 || clientes.contar() > 0) {
            log.info("Dados de exemplo: base ja populada, nada a fazer");
            return;
        }

        Espaco azul = espacos.salvar(new Espaco("Sala Azul", 30, new BigDecimal("150.00")));
        Espaco verde = espacos.salvar(new Espaco("Sala Verde", 12, new BigDecimal("90.00")));
        espacos.salvar(new Espaco("Auditorio", 120, new BigDecimal("420.00")));

        Cliente ana = clientes.salvar(new Cliente("Ana Souza", "ana@exemplo.com"));
        Cliente bruno = clientes.salvar(new Cliente("Bruno Lima", "bruno@exemplo.com"));
        clientes.salvar(new Cliente("Carla Dias", "carla@exemplo.com"));
